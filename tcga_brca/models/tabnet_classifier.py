"""
TabNet Classifier — Self-Supervised Pretraining + Full Evaluation
==================================================================
Two-phase training: unsupervised pretraining → supervised fine-tuning.
Computes all metrics: Accuracy, Macro F1, AUROC, 5-fold CV.
Compares attention-based feature importance with SHAP.
...
def train_tabnet_model(X_train, y_train, X_val, y_val, n_classes, seed=None):
    """Train a single TabNet model with self-supervised pretraining."""
    ...
    sample_weights = np.array([weights_arr[c] for c in y_train])
    ...
    pretrainer.fit(
        X_train=X_train, eval_set=[X_val],
...
def run_tabnet_cv(X, y, n_classes):
    """5-fold CV with pretraining per fold."""
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        model = train_tabnet_model(X_tr, y[tr_idx], X_val, y[val_idx], n_classes,
...
def plot_comparison(merged, overlap, save_dir):
    ...
    for gene in overlap:
        ...
        axes[0].annotate(gene, (row["shap_rank"], row["tabnet_rank"]), fontsize=6, alpha=0.8)
    ...
    top20_genes = list(set(
        merged.nsmallest(20, "tabnet_rank")["gene"].tolist() +
        merged.nsmallest(20, "shap_rank")["gene"].tolist()
    ))
    importance_compare = merged[merged["gene"].isin(top20_genes)].copy()
    importance_compare["TabNet_norm"] = importance_compare["tabnet_importance"] / importance_compare["tabnet_importance"].max()
    importance_compare["SHAP_norm"] = importance_compare["mean_abs_shap"] / importance_compare["mean_abs_shap"].max()
    ...
    axes[1].set_yticklabels(importance_compare["gene"].values, fontsize=7)
...
def main():
    ...
    # Train TabNet with pretraining
    print("\nTraining TabNet (self-supervised pretraining + fine-tuning)...")
    model = train_tabnet_model(X_train_s, y_train, X_test_s, y_test, n_classes)
    ...
    # 5-fold CV
    print("Running 5-fold CV (pretraining per fold)...")
    cv_acc, cv_f1, cv_auroc = run_tabnet_cv(expr.values, y, n_classes)
    ...
    ax.set_ylabel("True")
    ax.set_title(f"TabNet (Pretrained) — Confusion Matrix\nAccuracy: {acc:.4f} | Macro F1: {f1:.4f}")
    plt.tight_layout()
    ...
    # Feature importance + SHAP comparison
    ...
    summary = {
        ...
        "imbalance_strategy": "class_weights (sample_weight)",
        "pretraining": "self-supervised (pretraining_ratio=0.5)",
    }

    if result:
        merged, overlap, rho = result
        summary["top20_overlap_with_shap"] = len(overlap)
        summary["spearman_rho"] = float(rho)
        summary["consensus_genes"] = list(overlap)
        merged.to_csv(os.path.join(config.RESULTS_TABLES, "tabnet_vs_shap_comparison.csv"), index=False)
